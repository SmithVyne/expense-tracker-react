...
class CategoriesList extends Component {
  ...
  async almightyFetcher() {
    const {ADD_ALL_CATEGORIES, ADD_ALL_EXPENSES, date, currentUser} = this.props;
    const {id: user_id} = currentUser;
    this.setState({loading: true});

    await fetch(`${baseUrl}/categories?category_date=${date[0]}&user=${user_id}`)
    .then(response => response.json())
    .then(categories => ADD_ALL_CATEGORIES(categories) );
    
    this.setState({loading: false});

    await fetch(`${baseUrl}/expenses?expense_date=${date[0]}`)
    .then(response => response.json())
...
const mapStateToProps = ({currentUser, categories, date}) => ({currentUser, categories, date});
